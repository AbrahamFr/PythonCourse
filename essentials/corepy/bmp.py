...
def _int32_to_bytes(i):
    """Convert an integer to four bytes in little-endian fortmat."""
...
def write_grayscale(filename, pixels):
    ...
    with open(filename, 'wb') as bmp:
      ...
      for row in reversed(pixels):  # BMP files are bottom up
          row_data = bytes(row)
          bmp.write(row_data)
...
def dimensions(filename):
    ...
    Raises:
        ...
        OSError: if there was a prblem reading the file.
    """
...
def _bytes_to_int32(b):
    "Convert bytes object containing four bytes into an integer."
